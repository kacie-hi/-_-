...
def local_font_style(font_path="KakaoRegular.ttf"):
   ...
   <style>
   @font-face {{
       font-family: 'KakaoRegular'!important;
   ...
   html, body, [class*="css"] {{
       font-family: 'KakaoRegular', sans-serif !important;
   }}
   </style>
   """
...
def show_start_page():
   with st.container():
       # modal-wrapper 스타일을 block-container에 직접 입힘
       st.markdown("""
       <style>
       .block-container {
           background-color: #F7D358;
       ...
       <div class="typewriter-container">
           <p class="typewriter-text">당신의 마음을 금액으로 표현해드립니다 ♥</p>
       </div>
       <style>
...
def show_input_page():
   with st.container():
       # block-container에 직접 스타일 적용 (노란 배경, 모달 느낌)
       st.markdown("""
       <style>
       .block-container {
           background-color: #F7D358;
...
def show_result_page():
   ...
   with st.container():
       ...
       st.markdown(
           f'<span class="tag">{st.session_state.event_type}</span> '
           f'<span class="tag">{st.session_state.relationship}</span>',
           unsafe_allow_html=True
       )
       ...
       st.markdown(
           f'<p style="color: #452c22; font-size: 18px; font-weight: 600; margin-top: 30px;">'
           f'친밀도 점수: {results["intimacy_score"]}/100</p>',
           unsafe_allow_html=True
       )
       ...
       if results["special_factors"]:
           st.markdown('<h3 style="color: #D4A017;">✨ 특별 가산 요인</h3>', unsafe_allow_html=True)
           for factor in results["special_factors"]:
               st.markdown(f"• {factor}", unsafe_allow_html=True)


       st.markdown(f'💡 {results["funny_tip"]}', unsafe_allow_html=True)
